...
range-sharded browser format are implemented. Format v7 publishes only
...
under a 4 MiB decoded budget. The bootstrap is stored as gzip, keeping the Core
...
shards remain lossless, while formats v5 through v7 index only canonical headword results
as described in
